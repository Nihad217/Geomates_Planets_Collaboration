...
(defparameter *python3*
  (or (probe-file "/usr/bin/python3")
      (probe-file "/usr/local/bin/python3")
      (probe-file "/opt/homebrew/bin/python3")
      (probe-file "python3"))
  "binds to the first python3 binary that exists")

(defparameter *platforms* nil
  "List of platform rectangles (x y w h) collected from visicon.")

(defun clear-platforms ()
  (setf *platforms* nil))

(defun add-platform (x y w h)
  (push (list x y w h) *platforms*))

(defun platform-seen-p (x y w h)
  (member (list x y w h) *platforms* :test #'equal))

(defun call-dstar-nearest (sx sy diamonds)
  "Calls dStarSearch.py and returns (tx ty) or NIL on failure."
  (when (and *python3* (probe-file "dStarSearch.py") diamonds)
    (let* ((script (namestring (probe-file "dStarSearch.py")))
           (input (with-output-to-string (s)
                    (format s "~a ~a ~d" sx sy (length diamonds))
                    (dolist (d diamonds)
                      (format s " ~a ~a" (first d) (second d)))
                    (format s " ~d" (length *platforms*))
                    (dolist (p *platforms*)
                      (format s " ~a ~a ~a ~a" (first p) (second p) (third p) (fourth p)))
                    (format s "~%")))
           (output (with-output-to-string (out)
                     (run-program *python3* (list script)
                                  :input (make-string-input-stream input)
                                  :output out))))
      (multiple-value-bind (tx idx) (read-from-string output nil nil)
        (multiple-value-bind (ty idx2) (read-from-string output nil nil :start idx)
          (when (and (numberp tx) (numberp ty)) (list tx ty)))))))

(defun local-nearest-diamond (sx sy diamonds)
  "Fallback: compute nearest diamond by squared Euclidean distance."
  (let ((best nil)
...
(defun pick-target-with-dstar (sx sy d1x d1y d2x d2y d3x d3y)
  "Returns (tx ty), prefers dStarSearch.py, falls back to local distance."
  (let* ((diamonds (remove nil (list (and d1x d1y (list d1x d1y))
                                     (and d2x d2y (list d2x d2y))
                                     (and d3x d3y (list d3x d3y)))))
         (target (call-dstar-nearest sx sy diamonds)))
    (or target (local-nearest-diamond sx sy diamonds))))
...
(define-model lost-agent
  ...
  (chunk-type goal state intention
             ...
             platform-repeat
             platform-last-x platform-last-y platform-mode)
...
;; Warten, bis das Visicon befüllt ist
  ...
  ==>
     ...
     !eval! (clear-platforms)
     =goal>
       state scan-platforms
       platform-repeat 0
       platform-last-x -1000
       platform-last-y -1000
       platform-mode next-x
       intention nil)

  ;; Alle Plattformen (schwarze Polygone) einsammeln
  (p scan-platforms-next-x
     =goal>
       state scan-platforms
       intention nil
       platform-mode next-x
  ...
  ==>
     +visual-location>
       ...
       :attended nil
     =goal>
       intention attend-platform)

  (p scan-platforms-same-x
     =goal>
       state scan-platforms
       intention nil
       platform-mode same-x
       platform-last-x =lx
       platform-last-y =ly
     ?visual-location>
       state free
  ...
  ==>
     !bind! =w1 (if =w =w 0)
     !bind! =h1 (if =h =h 0)
     !bind! =seen (platform-seen-p =x =y =w1 =h1)
     !eval! (if (not =seen) (add-platform =x =y =w1 =h1) nil)
     !eval! (clear-buffer 'visual-location)
     !eval! (clear-buffer 'visual)
     =goal>
       state scan-platforms
  ...
  (p scan-platforms-same-x-failure
     =goal>
       state scan-platforms
       intention nil
       platform-mode same-x
  ...
  (p scan-platforms-next-x-failure
     =goal>
       state scan-platforms
       intention nil
       platform-mode next-x
  ...
  (p save-platform-visual-failure
     =goal>
       state scan-platforms
       intention save-platform
     ?visual>
...
;; Alle sichtbaren Objekte nacheinander über visual-location erfassen
  ...
  (p pick-target-disc
     =goal>
       ...
       diamond3-y =d3y
  ==>
     !bind! =target (pick-target-with-dstar =sx =sy =d1x =d1y =d2x =d2y =d3x =d3y)
     !bind! =tx (first =target)
     !bind! =ty (second =target)
     =goal>
       self-x =sx
       self-y =sy
       target-x =tx
