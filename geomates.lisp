			 ...
			 (let ((len (logand frame-info 127)))
			   ...
			   (let ((masking-key (loop for i from 1 to 4 collect (read-byte stream nil :eof)))
				 ...
				 (let ((msg (loop for data in payload
				   ...
				   (let ((msg-str (coerce msg 'string)))
				     (format *standard-output* "received from GUI:~a raw:~a~%" msg-str (list frame-header frame-info payload masking-key))
				     (sb-concurrency:enqueue msg-str *gui-command-queue*))))))))))))))
				...
				do
				   ...
				   (let (disc-pos-x disc-pos-y rect-pos-x rect-pos-y rect-rotation rect-width rect-height)
					...
					(let* ((*print-pretty* nil)
					 ...
					 (when (or (string= command "s") (string= command "skip"))
					   (format t "Skipping level...~%")
					   (setf diamonds nil)))))
				   (sleep 0.1))))))
	;; clean-up on exit
	(close disc-agent-stream)
